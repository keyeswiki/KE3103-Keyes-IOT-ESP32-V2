...
按照接线图接好线，外接电源，选择好正确的开发板板型（ESP32 Dev Module）和 适当的串口端口（COMxx），然后单击按钮![](media/cou0.png)上传示例代码至ESP32主控板。示例代码上传成功后，上电后，先点击一下木板房子⑦处(左侧)按键1，风扇开始转动，再点击木板房子⑨处(右侧)按键2进行调速，总共可以调3个不同转速，最后按一下木板房子⑦处(左侧)按键1，风扇停止。
