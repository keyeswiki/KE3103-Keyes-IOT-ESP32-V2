...
按照接线图接好线，外接电源，选择好正确的开发板板型（ESP32 Dev Module）和 适当的串口端口（COMxx），然后单击按钮![](media/upload2.png)上传示例代码至ESP32主控板。示例代码上传成功后，上电后。
...
点击 ![Img](./media/Serial1.png)，打开串口监视器，设置波特率为9600，如果成功连接上WiFi，串口监视器会打印出分配到的IP地址（假如串口监视器没打印出分配到的IP地址，可以按下ESP32主控板上的复位键重启，重新连接WiFi）。不同的网络（WiFi），IP地址不一样。
...
A. APP输入IP地址（LCD1602显示出分配到的IP地址，或点击IDE的串口监视器![](media/Serial1.png)，串口监视器窗口显示的IP地址）

B. 点击 **CONNECT** 连接IP地址。

C. 连接成功标志是跳出 Connected 字样，需要注意看。

D. IP地址连接上之后，然后就可以点击LED按钮，可以看到智能家居的LED被打开，再次点击LED按钮，LED被关闭；点击Fan按钮，风扇被打开，再次点击Fan按钮，Fan被关闭。如下图操作。
...
A. APP输入IP地址（LCD1602显示出分配到的IP地址，或点击IDE的串口监视器![Img](./media/Serial1.png)，串口监视器窗口显示的IP地址）。
